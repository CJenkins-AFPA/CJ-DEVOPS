...
generate_tag() {
    ...
    # Feature branch
        ...
        log_info "✨ Branch feature détectée: $branch"
    ...
    # Hotfix branch
        ...
        log_info "🔥 Branch hotfix détectée: $branch"
...
main() {
    ...
    # Push specific tag
    if ! docker_push_with_retry "${registry_url}/${image_name}" "$tag" "$RETRY_COUNT"; then
        exit 1
    fi
    
    # Push latest tag
    if ! docker_push_latest_with_retry "${registry_url}/${image_name}" "$RETRY_COUNT"; then
        exit 1
    fi
