...
uHub utilise **OAuth2 (Password flow v1) + JWT** pour les appels API. 
- **Access token JWT** :
...
Si au moins un user existe → 409 Conflict.
...
Réponse : 201 Run, puis mises à jour via statut/logs.
...
Réponse : 200, format compatible FullCalendar (title, start, end, allDay, extendedProps). 
...
Filtres : user, action, période, projet, etc.
